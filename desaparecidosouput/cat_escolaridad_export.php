...
require_once("include/cat_escolaridad_variables.php");
...
$layout = new TLayout("export", "OfficeDeliciousGray", "MobileDeliciousGray");
$layout->version = 2;
...
$page_layouts["cat_escolaridad_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idescolaridad"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["idescolaridad"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
